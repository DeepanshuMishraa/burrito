                ...
                VStack(spacing: 0) {
                    LazyVStack(alignment: .leading, spacing: 0) {
                        if sidebarSelection == .all {
                            Text("Coming up")
                                .font(.burritoUI(size: 28, weight: .medium))
                                .padding(.bottom, 16)
                            ...
                            Text(sectionTitle)
                                .font(.burritoUI(size: 26, weight: .bold))
                                .padding(.bottom, 20)
                        }
                    }
            ...
            VStack(spacing: 0) {
                HStack(spacing: 12) {
                    ...
                    TextField("Search notes and commands", text: $query)
                        ...
                        .font(.burritoUI(size: 16, weight: .regular))
        ...
        Button {
            ...
            HStack(spacing: 12) {
                ...
                if let shortcut = command.shortcut {
                    Text(shortcut)
                        .font(.burritoUI(size: 10, weight: .regular))
                        .foregroundStyle(.tertiary)
                }
            }
            .padding(.horizontal, 12)
                ...
                HStack(spacing: 2) {
                    Button {
                        ...
                        BurritoLabel("Speech to Text", systemImage: "waveform")
                            ...
                            .burritoElevation(.control, isActive: selectedTab == .speechToText)
                    }
                    .buttonStyle(.plain)
                    ...
                    Button {
                        ...
                        BurritoLabel("Text Models", systemImage: "sparkles")
                            ...
                            .burritoElevation(.control, isActive: selectedTab == .textModels)
                    }
                    .buttonStyle(.plain)
                }
                .padding(3)
            ...
            case .paused(let progress):
                VStack(alignment: .trailing, spacing: 6) {
                    ...
                    Text("\(max(1, Int(progress * 100)))% saved")
                        .font(.burritoUI(size: 9, weight: 400, relativeTo: .caption2))
                        .foregroundStyle(.tertiary)
                }
    ...
    var body: some View {
        Button(action: action) {
            BurritoIcon(name: "plus", size: 9)
                ...
                .burritoElevation(.control, isActive: isHovered)
        }
        .buttonStyle(.plain)
        .onHover { isHovered = $0 }
        .animation(.easeOut(duration: 0.14), value: isHovered)
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            if count > 0 {
                Text("\(count)")
                    .font(
                        .burritoUI(size: 11, weight: 400, relativeTo: .caption)
                            .monospacedDigit()
                    )
                    .foregroundStyle(.tertiary)
            }
    ...
    var body: some View {
        Button {
            ...
            HStack(spacing: 8) {
                ...
                .frame(width: 18)
                Text(title)
                    .font(.burritoUI(size: 13, weight: isSelected ? 450 : 400))
                    .lineLimit(1)
                Spacer()
                if count > 0 {
                    Text("\(count)")
        ...
        HStack(spacing: 12) {
            VStack(alignment: .leading, spacing: 2) {
                Text(event.startDate.formatted(date: .omitted, time: .shortened))
                    .font(.burritoUI(size: 12, weight: 450).monospacedDigit())
                    .lineLimit(1)
                    .fixedSize()
    ...
    var body: some View {
        VStack(spacing: 8) {
            ...
            Text(isTrash ? "Trash is empty" : isSearching ? "Nothing found" : "A quiet start")
                .font(.burritoUI(size: 13, weight: .semibold, relativeTo: .headline))
...
private struct MarkdownNoteContent: View {
    ...
    private func blockView(_ block: MarkdownDocument.Block) -> some View {
                ...
                ForEach(Array(items.enumerated()), id: \.offset) { index, item in
                    HStack(alignment: .firstTextBaseline, spacing: 10) {
                        ZStack {
                            ...
                            Text("\(index + 1)")
                                .font(.burritoUI(size: 10, weight: 600))
                                .foregroundStyle(BurritoTheme.accent)
                        }
                        inlineText(item)
                            .font(.burritoUI(size: 14, weight: 400))
        ...
        case .quote(let kind, let text):
            HStack(alignment: .top, spacing: 10) {
                BurritoIcon(
                    name: kind == .warning ? "shield.exclamation" : "info",
                    size: 14
                )
                    .foregroundStyle(BurritoTheme.accent)
                    .padding(.top, 2)
        ...
        case .code(let text):
            ScrollView(.horizontal) {
                Text(text)
                    .font(.burritoUI(size: 12.5, weight: 400))
                                ...
                                VStack(alignment: .leading, spacing: 4) {
                                    HStack(spacing: 6) {
                                        Text(speakerLabel(for: passage.source))
                                            .font(.burritoUI(size: 10, weight: 550))
                                            .foregroundStyle(
                    ...
                    Button {
                        ...
                        HStack(spacing: 10) {
                            Text("@")
                                ...
                                .frame(width: 18)
                            Text(document.title)
                                .font(.burritoUI(size: 12, weight: 450, relativeTo: .callout))
                                .foregroundStyle(.primary)
                                .lineLimit(1)
                            Spacer()
                        ...
                        .burritoElevation(
                            .control,
                            isActive: index == session.mentionSelectionIndex
                        )
                    }
                    .buttonStyle(.plain)
                }
...
private struct NoteEditingView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            TextEditor(text: $userNotes)
                .font(.burritoUI(size: 14, weight: .regular))
                .lineSpacing(5)
                .scrollContentBackground(.hidden)
                .burritoThinScrollers()
    ...
    var body: some View {
        Button(action: action) {
            BurritoIcon(name: systemImage, size: 13)
                ...
                .burritoElevation(.control, isActive: isSelected)
        }
        .buttonStyle(.plain)
        ...
        VStack(spacing: 0) {
                ...
                HStack(spacing: 12) {
                    ...
                    if selectedTab != .chat {
                        ...
                        TextField("Untitled note", text: titleBinding)
                            .textFieldStyle(.plain)
                            .font(.burritoUI(size: 15, weight: 450))
                            .foregroundStyle(.primary)
            ...
            if !isRecordingThisNote && selectedTab != .chat {
                HStack(spacing: 8) {
                    ...
                    Text("·")
                        .font(.burritoUI(size: 11, weight: .regular))
                        .foregroundStyle(.tertiary)
...
private struct TranscriptEditor: View {
    ...
    var body: some View {
        ScrollViewReader { proxy in
            ScrollView {
                ...
                .padding(.top, 88)
                    ...
                    ForEach(
                        ...
                        .burritoElevation(
                            .control,
                            isActive: focusedSegmentID == segment.id
            ...
            .onChange(of: note.transcriptData) { _, transcriptData in
                guard let updatedSegments = try? JSONDecoder().decode(
                    [TranscriptSegment].self,
                    from: transcriptData
